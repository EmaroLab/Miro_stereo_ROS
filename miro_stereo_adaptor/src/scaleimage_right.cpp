/**
 * \file scaleimage_right.cpp
 * \brief A node to scale and republish the Image from Miro's Right eye camera. 
 ...
 * \param[in] ...
 * 
 * Subscribes to: /miro/rob01/platform/camr<BR>
 * 
 * Publishes to: /miro_scaledimage/right/image_raw<BR>
...
public:
    ...
    : it_(nh_)
  {
    ...
    image_sub_ = it_.subscribe("/miro/rob01/platform/camr", 1,
      &ImageConverter::imageCb, this);
    image_pub_ = it_.advertise("/miro_scaledimage/right/image_raw", 1);
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
	...
	cv_ptr->image=cv_ptr->image(cv::Range(0,cv_ptr->image.rows),cv::Range(0,140));
...
{
  ros::init(argc, argv, "scaleimage_right");
  ImageConverter ic;
  ros::spin();
  return 0;
}
